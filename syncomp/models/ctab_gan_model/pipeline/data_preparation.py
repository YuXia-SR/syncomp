...
class DataPrep(object):
    ...
    def __init__(self, raw_df: pd.DataFrame, categorical: list, log:list, mixed:dict, general:list, non_categorical:list, integer:list, type:dict):
        ...
        problem = list(type.keys())[0] if len(type)>0 else None
        target_col = list(type.values())[0] if len(type)>0 else None
        if problem:
            y_real = raw_df[target_col]
            X_real = raw_df.drop(columns=[target_col])
            
            # if problem=="Classification":
            #     X_train_real, _, y_train_real, _ = model_selection.train_test_split(X_real ,y_real, test_size=test_ratio, stratify=y_real,random_state=self.random_state)
            # else:
            #     X_train_real, _, y_train_real, _ = model_selection.train_test_split(X_real ,y_real, test_size=test_ratio,random_state=self.random_state) 
